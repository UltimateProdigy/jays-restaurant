...
const SteaksPage: React.FC = () => {
  const navigate = useNavigate();

  const steaks = desertMenu.filter(
    (food: FoodItem) => food.category === "Steaks & Premium Cuts"
  );

  const categoryColor = "from-amber-400 to-orange-500";
  const categoryIcon = "🥩";
    ...
    >
      <div className="max-w-6xl mx-auto text-center mb-16">
        ...
        <p className="text-xl text-slate-300 max-w-2xl mx-auto leading-relaxed">
          Delicious delicacy for any occasion
        </p>
        <div
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
          {steaks.map((food: FoodItem, index: number) => (
            <div
              key={food.id}
