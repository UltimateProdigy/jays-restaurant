...
const SeafoodsPage: React.FC = () => {
  const navigate = useNavigate();

  const sea = desertMenu.filter(
    (food: FoodItem) => food.category === "Seafood Delicacies"
  );

  const categoryColor = "from-blue-400 to-indigo-500";
  const categoryIcon = "🐟";
    ...
    >
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
          {sea.map((food: FoodItem, index: number) => (
            <div
              key={food.id}
