...
const FingersPage: React.FC = () => {
  const navigate = useNavigate();
  
  const fingers = foodMenu.filter((food: FoodItem) => food.category === "Fingers");

  const categoryColor = "from-green-500 to-teal-500";
  const categoryIcon = "🍗";
    ...
    >
      <div className="max-w-6xl mx-auto text-center mb-16">
        ...
        <p className="text-xl text-slate-300 max-w-2xl mx-auto leading-relaxed">
          Delicious finger foods perfect for any occasion
        </p>
        <div className={`w-32 h-1 bg-gradient-to-r ${categoryColor} mx-auto mt-6 rounded-full`}></div>
      </div>

      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
          {fingers.map((food: FoodItem, index: number) => (
            <div
              key={food.id}
      ...
      <div className="max-w-4xl mx-auto mt-24 text-center">
        <div className="bg-gradient-to-r from-green-500/20 to-teal-500/20 backdrop-blur-xl border border-green-500/30 rounded-3xl p-8">
          ...
          <p className="text-slate-300 mb-6">
            All our finger foods are prepared with fresh, locally sourced ingredients
