...
const SoupsPage: React.FC = () => {
  const navigate = useNavigate();

  const soups = foodMenu.filter((food: FoodItem) => food.category === "Soups");

  const categoryColor = "from-orange-500 to-red-500";
  const categoryIcon = "🥣";
    ...
    >
      <div className="max-w-6xl mx-auto text-center mb-16">
        ...
        <p className="text-xl text-slate-300 max-w-2xl mx-auto leading-relaxed">
          Traditional Nigerian soups prepared with authentic ingredients
        </p>
        <div
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
          {soups.map((food: FoodItem, index: number) => (
            <div
              key={food.id}
      ...
      <div className="max-w-4xl mx-auto mt-24 text-center">
        <div className="bg-gradient-to-r from-orange-500/20 to-red-500/20 backdrop-blur-xl border border-orange-500/30 rounded-3xl p-8">
          ...
          <p className="text-slate-300 mb-6">
            All our soups are prepared with fresh, locally sourced ingredients
      ...
      <div className="max-w-6xl mx-auto mt-12 text-center">
        <button
          ...
          className="px-8 py-3 bg-white/10 hover:bg-white/20 border border-white/20 hover:border-orange-500/50 text-white rounded-full transition-all duration-300 backdrop-blur-md font-medium"
        >
          ← Back to Starters
        </button>
      </div>
